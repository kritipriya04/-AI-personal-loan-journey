...
function Screen1({
  ...
  const allFilled =
    ...
    Number(form.loanAmount) >= 50000 &&
    Number(form.loanAmount) <= 1000000;
    ...
    <>
      ...
      >
        <div className="px-5 pt-5 pb-6 flex flex-col gap-4">
          ...
          <Field
            label="Loan Amount"
            hint="Enter the amount you want to borrow · ₹50,000–₹10,00,000"
          >
            <div className="flex items-center bg-white rounded-[14px] overflow-hidden"
              style={{ border: "1.5px solid #e2e6f0" }}
            >
              <span className="px-4 text-[15px] font-semibold text-[#4b5a80]">
                ₹
              </span>
              <input
                type="number"
                ...
                className="flex-1 bg-white text-[15px] py-3.5 pr-4 outline-none"
              />
            </div>
            {form.loanAmount && (
              <p className="text-[11px] text-[#6b7280]">
                {Number(form.loanAmount) >= 50000 &&
                Number(form.loanAmount) <= 1000000
                  ? `Selected: ${formatCurrency(form.loanAmount)}`
                  : "Please enter an amount between ₹50,000 and ₹10,00,000."}
              </p>
            )}
          </Field>
...
function calculateEMI(principal: string, annualRate = 14, months = 24) {
      ...
      >
        <div className="px-5 pt-5 pb-6 flex flex-col gap-4">
          ...
          <ReviewSection title="Personal Details">
            ...
            <ReviewRow label="Mobile" value={formatMobile(form.mobile)} />
            <ReviewRow label="Date of birth" value={formatDateOfBirth(form.dob)} />
            <ReviewRow
              label="Employment"
              value={form.employment || "—"}
            />
            <ReviewRow label="Monthly income" value={formatIncome(form.income)} />
          </ReviewSection>
          ...
          <ReviewSection title="Loan Details">
            ...
            <ReviewRow
              label="Estimated EMI"
              value={
                calculateEMI(form.loanAmount)
                  ? `${formatCurrency(String(calculateEMI(form.loanAmount)))} / month`
                  : "—"
              }
            />
    ...
    <>
      ...
      >
        ...
        <p className="text-blue-100 text-[11px] mt-1">
          Loan amount: {formatCurrency(form.loanAmount)}
        </p>
      </div>
  ...
  >([
    {
      from: "ai",
      text:
        `Hi ${name || "there"}. I'm Loan Guide. I can explain your application status, loan terms and what happens next. I won't make lending decisions.`,
    },
  ]);
